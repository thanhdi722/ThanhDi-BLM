...
const ProductWatch: React.FC = () => {
  ...
  const [visibleCount, setVisibleCount] = useState<number>(5) // Initial visible product count
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      if (window.innerWidth < 768) {
        ...
        setVisibleCount(5) // Show 10 items if screen width >= 768px
      }
    }
          ...
          <div className="upgrade-hot-wrap">
            {dataTitle ? (
              dataTitle?.data?.Slider?.items[0]?.Banner?.items
                .filter((item: any) => item.media_alt.includes('title-banner-thang-3-watch'))
                  ...
                  <div key={tab}>
                    <button
                      ...
                      style={{
                        ...
                        border: activeTab === tab ? '1px solid #ffd400' : '1px solid #ccc',
                        padding: '10px 20px',
                        borderRadius: '5px',
                        cursor: 'pointer',
                        minWidth: '150px',
