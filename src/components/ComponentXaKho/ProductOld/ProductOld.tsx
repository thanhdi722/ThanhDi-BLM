...
import { useProductSaleDataMAYSSXaKho } from "../hook/dataPK";
...
const AppleList: React.FC = () => {
  const { data } = useProductSaleDataMAYSSXaKho();
  const filteredDatassss = data?.filter(
    (item: any) => item.title === "Sp Pad Xả Kho"
  );
                ...
                >
                  {dataTitle ? (
                    dataTitle?.data?.Slider?.items[0]?.Banner?.items
                      .filter((item: any) =>
                        item.name.includes("title sản phẩm máy 99 page xả kho")
                      )
