...
const AppleList: React.FC = () => {
  ...
  const filteredDataPK12K = dataPK12K?.filter(
    (item: any) => item.title === "SP PK 12K 24-12"
  );

  const [activeTab, setActiveTab] = useState<string>("12k");
              ...
              <div>
                <div className="women-decor" style={{ paddingBottom: "20px" }}>
                  {dataTitle ? (
                    dataTitle?.data?.Slider?.items[0]?.Banner?.items
                      .filter((item) =>
                        item.name.includes(
                          "title phụ kiện đón giáng sinh 24/12"
                        )
                      )
                      .map((item, index) => (
                        <div key={index}>
                ...
                <div style={{ display: "flex", justifyContent: "center" }}>
                  <div className="tab-buttons">
                    <button
                      className={activeTab === "12k" ? "active" : ""}
                      onClick={() => handleTabChange("12k")}
                    >
                      Phụ kiện 12K
                    </button>
                    <button
