...
const AndroidList: React.FC = () => {
  ...
  const filteredDatassssXIAOMI = dataAndroidXIAOMI?.filter((item: any) => item.title === 'SP XIAOMI 24/12')
  const { data: dataAndroidOppo } = useProductSaleDataAndroidOppo()
  const filteredDatassssOppo = dataAndroidOppo?.filter((item: any) => item.title === 'SP OPPO 24/12')
                ...
                <div className="women-decor" style={{ padding: '10px 0px' }}>
                  {dataTitle ? (
                    dataTitle?.data?.Slider?.items[0]?.Banner?.items
                      .filter((item) => item.name.includes('title android laptop đón giáng sinh 24/12'))
                      .map((item, index) => (
                        <div key={index}>
